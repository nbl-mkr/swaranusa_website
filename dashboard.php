...
if (isset($_POST["hapus_belajar_id"])) {
    $hapus = $conn->prepare("DELETE FROM konten_belajar WHERE id = ?");
...
$total_result_belajar = $conn->query("SELECT COUNT(*) as total FROM konten_belajar")->fetch_assoc()["total"];
$total_pages_belajar = ceil($total_result_belajar / $per_page);

$result_belajar = $conn->query("SELECT * FROM konten_belajar ORDER BY $sort $order LIMIT $per_page OFFSET $offset");
            ...
            <section class="content-area">
                ...
                <div class="tab">
                    <button class="tab-btn <?= $tab === 'jelajahi' ? 'active' : '' ?>"
                        onclick="window.location.href='dashboard.php?tab=jelajahi'">Jelajahi</button>
                    <button class="tab-btn <?= $tab === 'belajar' ? 'active' : '' ?>"
                        onclick="window.location.href='dashboard.php?tab=belajar'">Belajar</button>
                </div>
                ...
                <div class="dashboard-container">
                    ...
                    <?php else: ?>
                        <div class="container-top">
                            ...
                            <div class="card">
                                <h3>Total Histori</h3>
                                <p><?= $total_histori_belajar ?></p>
                            </div>
                            <div class="card">
                                <h3>Konten Baru Bulan Ini</h3>
                                <p><?= $konten_belajar_bulan_ini ?></p>
                            </div>
                        </div>
                        ...
                        <div class="container-bottom">
                            ...
                            <div class="table-controls">
                                ...
                                <select id="filter-kategori-belajar">
                                    <option value="">Semua Kategori</option>
                                    <?php
                                    $kategori_belajar_list = $conn->query("SELECT DISTINCT kategori FROM konten_belajar ORDER BY kategori ASC");
